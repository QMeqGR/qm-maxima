...
;; Just use set_display('ascii)$ instead of this stuff
;;(setf (get '$bra 'dissym) '((#\<) #\|))
...
;; this is a one character change from the code in tpdisp.lisp,
;; "(setq dissym '(#\space #\* #\space) form (car l)) )"
;; and changes the otimes for * because the otimes doesn't
;; obey monospacing in wxmaxima apparently.
(displa-def motimes dim-motimes)
(defprop munaryotimes (#\⊗ #\space) dissym)
(defprop cdot (#\· #\space) dissym)
...
;; The otimes that works is unicode is U+2A02. All others fail to produce correct spacing!
